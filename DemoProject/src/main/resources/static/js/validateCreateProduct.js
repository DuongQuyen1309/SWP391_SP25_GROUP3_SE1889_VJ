...
async function checkProductName(name) {
    const response = await fetch(`/product/checkname?name=${encodeURIComponent(name)}`);
    if (!response.ok) {
...
async function validateForm(event) {
    ...
    try {
        isNameExists = await checkProductName(nameInput.value);
    ...
    if (isNameValid && isPriceValid && isDescriptionValid && !isNameExists) {
        Swal.fire({
            ...
            text: 'Bạn có chắc chắn muốn tạo sản phẩm mới này?',
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            confirmButtonText: 'Tạo sản phẩm',
...
document.addEventListener("DOMContentLoaded", () => {
    const customerForm = document.getElementById("productValid");
    customerForm.addEventListener("submit", (event) => validateForm(event));
});
